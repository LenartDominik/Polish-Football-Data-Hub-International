# 🏗️ Architektura Systemu - Polish Football Data Hub International

## 📊 Diagram Kompletny

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│                         👤 UŻYTKOWNIK (Przeglądarka)                            │
│                                                                                 │
└────────────────────────────────────┬────────────────────────────────────────────┘
                                     │
                                     │ HTTPS
                                     ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│                    ☁️ STREAMLIT CLOUD (Frontend Layer)                          │
│                    https://yourapp.streamlit.app                                │
│                    🆓 FREE TIER: Unlimited users, 1GB RAM                       │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                                                                         │   │
│  │  📱 streamlit_app_cloud.py                                              │   │
│  │                                                                         │   │
│  │  ┌───────────────┐  ┌──────────────┐  ┌─────────────────────┐         │   │
│  │  │  Dashboard    │  │  Player      │  │  Comparison         │         │   │
│  │  │  - Tabela     │  │  Cards       │  │  - Side by side     │         │   │
│  │  │  - Filtry     │  │  - Details   │  │  - Charts (Plotly)  │         │   │
│  │  │  - Search     │  │  - Stats     │  │  - xG comparison    │         │   │
│  │  └───────────────┘  └──────────────┘  └─────────────────────┘         │   │
│  │                                                                         │   │
│  │  Dependencies (requirements-streamlit.txt):                            │   │
│  │  - streamlit==1.51.0                                                   │   │
│  │  - pandas==2.3.3                                                       │   │
│  │  - plotly==5.18.0                                                      │   │
│  │  - requests==2.32.5                                                    │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Secrets (.streamlit/secrets.toml):                                            │
│  ├── BACKEND_API_URL = "https://polish-players-tracker.onrender.com"           │
│  └── DEBUG = false                                                              │
│                                                                                 │
└────────────────────────────────────┬────────────────────────────────────────────┘
                                     │
                                     │ REST API (HTTPS)
                                     │ GET /api/players
                                     │ GET /api/stats/{player_id}
                                     │ GET /api/comparison
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│                    🚀 RENDER.COM (Backend Layer)                                │
│                    https://polish-players-tracker-xxxx.onrender.com             │
│                    🆓 FREE TIER: 750h/month, 512MB RAM, 1GB Disk                │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                                                                         │   │
│  │  🔧 FastAPI Backend (app/backend/main.py)                               │   │
│  │                                                                         │   │
│  │  ┌────────────────────────────────────────────────────────────────┐    │   │
│  │  │  API Endpoints:                                                │    │   │
│  │  │  ├── GET  /health                 (Health check)              │    │   │
│  │  │  ├── GET  /api/players            (Lista graczy)              │    │   │
│  │  │  ├── GET  /api/players/{id}       (Szczegóły gracza)          │    │   │
│  │  │  ├── GET  /api/stats              (Statystyki)                │    │   │
│  │  │  ├── GET  /api/comparison         (Porównanie)                │    │   │
│  │  │  └── GET  /api/matches            (Mecze)                     │    │   │
│  │  └────────────────────────────────────────────────────────────────┘    │   │
│  │                                                                         │   │
│  │  Middleware:                                                            │   │
│  │  ├── CORS (allow all origins)                                          │   │
│  │  └── Logging                                                            │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│                                     │                                           │
│                                     ▼                                           │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                                                                         │   │
│  │  💾 PostgreSQL Database (Supabase)                                       │   │
│  │  Cloud-hosted with automatic backups (500MB free)                       │   │
│  │                                                                         │   │
│  │  Tabele:                                                                │   │
│  │  ┌──────────────────┐  ┌─────────────────────┐  ┌──────────────────┐  │   │
│  │  │  players         │  │  competition_stats  │  │  player_matches  │  │   │
│  │  │  ───────────────│  │  ─────────────────  │  │  ──────────────  │  │   │
│  │  │  - id           │  │  - player_id        │  │  - player_id     │  │   │
│  │  │  - name         │  │  - season           │  │  - match_date    │  │   │
│  │  │  - position     │  │  - competition_type │  │  - opponent      │  │   │
│  │  │  - team         │  │  - games            │  │  - competition   │  │   │
│  │  │  - nationality  │  │  - goals            │  │  - minutes       │  │   │
│  │  │  - fbref_id     │  │  - assists          │  │  - goals         │  │   │
│  │  │  - created_at   │  │  - xg, xa, npxg     │  │  - assists       │  │   │
│  │  └──────────────────┘  └─────────────────────┘  └──────────────────┘  │   │
│  │                                                                         │   │
│  │  ┌──────────────────┐  ┌─────────────────────┐                         │   │
│  │  │  season_stats    │  │  goalkeeper_stats   │                         │   │
│  │  │  ───────────────│  │  ─────────────────  │                         │   │
│  │  │  - player_id    │  │  - player_id        │                         │   │
│  │  │  - season       │  │  - season           │                         │   │
│  │  │  - league       │  │  - saves            │                         │   │
│  │  │  - total stats  │  │  - clean_sheets     │                         │   │
│  │  └──────────────────┘  └─────────────────────┘                         │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│                                     ▲                                           │
│                                     │                                           │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                                                                         │   │
│  │  🤖 APScheduler (Background Jobs)                                       │   │
│  │                                                                         │   │
│  │  ┌────────────────────────────────────────────────────────────────┐    │   │
│  │  │  Zadania:                                                      │    │   │
│  │  │  ├── sync_all_players()    co 6h    ┐                         │    │   │
│  │  │  ├── sync_match_logs()     co 12h   ├─ Data update    │    │   │
│  │  │  ├── cleanup_old_data()    co 24h   │                         │    │   │
│  │  │  └── health_check()        co 10min ┘  (keep-alive)           │    │   │
│  │  └────────────────────────────────────────────────────────────────┘    │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│                                     │                                           │
│                                     ▼                                           │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                                                                         │   │
│  │  🎭 Playwright Scraper (services/fbref_playwright_scraper.py)           │   │
│  │                                                                         │   │
│  │  Flow:                                                                  │   │
│  │  1. Launch headless Chromium                                           │   │
│  │  2. Navigate to FBref.com/players/{fbref_id}                           │   │
│  │  3. Parse HTML (BeautifulSoup)                                         │   │
│  │  4. Extract: stats, matches, xG/xA data                                │   │
│  │  5. Save to database                                                   │   │
│  │                                                                         │   │
│  │  Features:                                                              │   │
│  │  ├── Stealth mode (evade detection)                                    │   │
│  │  ├── Retry logic (3 attempts)                                          │   │
│  │  ├── Rate limiting (1 req/5s)                                          │   │
│  │  └── Error handling & logging                                          │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└────────────────────────────────────┬────────────────────────────────────────────┘
                                     │
                                     │ HTTPS (Web Scraping)
                                     ▼
                            ┌──────────────────┐
                            │                  │
                            │  📊 FBref.com    │
                            │  (Źródło danych) │
                            │                  │
                            └──────────────────┘
```

---

## 🔄 Przepływ Danych

### 1. Aktualizacja Danych (Background)
```
Scheduler (co 6h)
    ↓
Playwright Scraper
    ↓
FBref.com → Parse HTML → Extract Stats
    ↓
PostgreSQL Database (Supabase)
```

### 2. Wyświetlanie Danych (User Request)
```
User (Browser)
    ↓
Streamlit Cloud (Frontend)
    ↓
REST API Request → Render Backend (FastAPI)
    ↓
Query PostgreSQL Database
    ↓
Return JSON Response
    ↓
Streamlit Cloud → Render UI
    ↓
User sees Dashboard
```

---

## 📦 Komponenty

### Frontend (Streamlit Cloud)
| Komponent | Plik | Opis |
|-----------|------|------|
| **Main App** | `streamlit_app_cloud.py` | Główna aplikacja UI |
| **Pages** | `pages/2_⚖️_compare_players.py` | Porównanie graczy |
| **Config** | `.streamlit/config.toml` | Konfiguracja Streamlit |
| **Secrets** | `.streamlit/secrets.toml` | Zmienne środowiskowe |

### Backend (Render)
| Komponent | Plik | Opis |
|-----------|------|------|
| **API** | `app/backend/main.py` | FastAPI endpoints |
| **Models** | `app/backend/models/*.py` | SQLAlchemy models |
| **Routers** | `app/backend/routers/*.py` | API route handlers |
| **Services** | `app/backend/services/*.py` | Business logic |
| **Scraper** | `services/fbref_playwright_scraper.py` | Web scraping |

### Database (PostgreSQL)
| Tabela | Rekordy | Opis |
|--------|---------|------|
| **players** | ~100 | Podstawowe info o graczach |
| **competition_stats** | ~1000 | Statystyki per sezon/rozgrywki |
| **player_matches** | ~5000 | Szczegółowe mecze z datami |
| **season_stats** | ~300 | Statystyki sezonowe agregowane |
| **goalkeeper_stats** | ~50 | Specjalne statystyki bramkarzy |

---

## 🌐 Network Flow

```
┌─────────────┐
│   User      │
│  Browser    │
└──────┬──────┘
       │
       │ HTTPS (443)
       │ GET https://yourapp.streamlit.app
       ▼
┌─────────────────────┐
│  Streamlit Cloud    │
│  CDN/Load Balancer  │
└──────┬──────────────┘
       │
       │ Render App
       ▼
┌─────────────────────┐
│  streamlit_app.py   │
│  - Load secrets     │
│  - BACKEND_API_URL  │
└──────┬──────────────┘
       │
       │ HTTPS API Call
       │ GET https://backend.onrender.com/api/players?limit=10
       ▼
┌─────────────────────┐
│  Render.com         │
│  Load Balancer      │
└──────┬──────────────┘
       │
       │ Route to container
       ▼
┌─────────────────────┐
│  FastAPI Backend    │
│  Docker Container   │
└──────┬──────────────┘
       │
       │ SQLAlchemy ORM
       │ SELECT * FROM players LIMIT 10
       ▼
┌─────────────────────┐
│  SQLite Database    │
│  /data/players.db   │
└─────────────────────┘
```

---

## 💾 Storage

### Persistent Disk (Render)
```
/data/
├── players.db          (główna baza ~50MB)
├── logs/
│   ├── app.log        (logi aplikacji)
│   └── scraper.log    (logi scrapera)
└── backups/
    └── players.db.backup (backup co 24h)
```

### Streamlit Cloud (Ephemeral)
```
/app/
├── streamlit_app_cloud.py
├── requirements-streamlit.txt
├── .streamlit/
│   └── config.toml
└── cache/              (cache danych, znika przy redeploy)
```

---

## 🔐 Security & Authentication

### Current (v0.7.3)
```
┌─────────────┐
│   User      │  No authentication
└──────┬──────┘  (Public access)
       │
       ▼
┌─────────────┐
│  Frontend   │  No API keys required
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  Backend    │  CORS: allow all origins
└─────────────┘  (Open API)
```

### Future (Production)
```
┌─────────────┐
│   User      │  OAuth login
└──────┬──────┘  (Google/GitHub)
       │
       ▼
┌─────────────┐
│  Frontend   │  JWT token in headers
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  Backend    │  API Key validation
└─────────────┘  Rate limiting
```

---

## 📊 Performance

### Caching Strategy

**Frontend (Streamlit):**
```python
@st.cache_data(ttl=3600)  # 1 hour
def get_players():
    return requests.get(f"{BACKEND_API_URL}/api/players")

@st.cache_data(ttl=300)  # 5 minutes
def get_player_stats(player_id):
    return requests.get(f"{BACKEND_API_URL}/api/stats/{player_id}")
```

**Backend (FastAPI):**
```python
# In-memory cache
cache = TTLCache(maxsize=100, ttl=3600)

@app.get("/api/players")
@cached(cache)
def get_players():
    return db.query(Player).all()
```

### Response Times
| Endpoint | Cold Start | Warm | Cached |
|----------|-----------|------|--------|
| `/health` | 30-60s | <100ms | - |
| `/api/players` | 30-60s | 200-500ms | <50ms |
| `/api/stats/{id}` | 30-60s | 300-800ms | <50ms |
| Frontend load | 2-5s | 1-2s | <1s |

---

## 🔄 Deployment Pipeline

```
┌──────────────────────────────────────────────────────────────┐
│  Developer                                                   │
└────────────────────┬─────────────────────────────────────────┘
                     │
                     │ git push origin main
                     ▼
┌──────────────────────────────────────────────────────────────┐
│  GitHub Repository                                           │
│  - Webhook triggers deploy                                   │
└────────────┬────────────────────────────┬────────────────────┘
             │                            │
             │ Webhook                    │ Webhook
             ▼                            ▼
┌─────────────────────────┐  ┌──────────────────────────────┐
│  Streamlit Cloud        │  │  Render.com                  │
│  1. Clone repo          │  │  1. Clone repo               │
│  2. pip install -r      │  │  2. docker build             │
│     requirements-       │  │  3. docker push              │
│     streamlit.txt       │  │  4. Deploy container         │
│  3. streamlit run       │  │  5. Health check             │
│  4. Deploy (1-2 min)    │  │  6. Route traffic (5-10 min) │
└─────────────────────────┘  └──────────────────────────────┘
             │                            │
             ▼                            ▼
      ✅ Live                       ✅ Live
```

---

## 🆚 Architektura: SQLite vs PostgreSQL

### Option A: SQLite (Current)
```
┌─────────────┐
│  Backend    │
│  Container  │
│     ↓       │
│  SQLite     │  Pros: Simple, fast, no config
│  File       │  Cons: Single writer, local only
└─────────────┘
```

### Option B: PostgreSQL (Production)
```
┌─────────────┐      ┌──────────────┐
│  Backend    │ ───→ │  PostgreSQL  │  Pros: Multi-user, ACID, backups
│  Container  │      │  (Supabase)  │  Cons: More complex, network latency
└─────────────┘      └──────────────┘
```

---

## 📈 Scalability

### Current Limits (Free Tier)
| Resource | Limit | Enough For |
|----------|-------|------------|
| Database Size | 1 GB | ~100k players |
| RAM (Backend) | 512 MB | ~100 concurrent users |
| RAM (Frontend) | 1 GB | ~500 concurrent users |
| CPU (Backend) | Shared | Light API calls |
| CPU (Frontend) | Shared | Dashboard rendering |

### When to Upgrade?
- **>100 players** → Consider PostgreSQL
- **>1000 concurrent users** → Paid Streamlit plan
- **>10k requests/day** → Paid Render plan
- **Need SLA** → Production infrastructure

---

## 🎯 Podsumowanie

**Aktualna Architektura (v0.7.3):**
- ✅ Frontend: Streamlit Cloud (Free)
- ✅ Backend: Render.com (Free)
- ✅ Database: SQLite (Persistent Disk)
- ✅ Scraper: Playwright (Background jobs)
- ✅ Cost: **$0/month**
- ✅ Uptime: 24/7 (z cold start)

**Idealna dla:**
- 📚 Projekty edukacyjne
- 💼 Portfolio
- 🧪 Prototypy
- 👥 <1000 użytkowników/dzień

**Gotowe na:**
- 🚀 Deployment (15 minut)
- 🔄 Auto-updates (GitHub push)
- 📊 Monitoring (Dashboard)
- 🐛 Debugging (Logs)

---

**Data:** 25.11.2025  
**Wersja:** v0.7.3  
**Status:** ✅ Production Ready
