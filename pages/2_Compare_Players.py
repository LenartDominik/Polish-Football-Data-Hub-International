...
# Get API URL - priority: st.secrets > env vars > localhost
def get_api_base_url():
    """Get API base URL from secrets, env vars, or default to localhost"""
    # Try Streamlit secrets first (for Streamlit Cloud)
    try:
        api_url = st.secrets.get("BACKEND_API_URL", None)
        if api_url:
            return api_url
